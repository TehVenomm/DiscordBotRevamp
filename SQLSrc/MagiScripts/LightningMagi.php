<?php
    function insertLightningMagi() {
        ...
        if (($handle = fopen($linkLightningMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $lightningMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $lightningMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $lightningMagiQuery) 
                        {
                            echo 'Caught exception: ',  $lightningMagiQuery->getMessage(), "</br>";
                        }
