<?php
    function insertHybridMagi() {
        ...
        if (($handle = fopen($linkHybridMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $hybridMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $hybridMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $hybridMagiQuery) 
                        {
                            echo 'Caught exception: ',  $hybridMagiQuery->getMessage(), "</br>";
                        }
