<?php
    function insertLightMagi() {
        ...
        if (($handle = fopen($linkLightMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $lightMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $lightMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $lightMagiQuery) 
                        {
                            echo 'Caught exception: ',  $lightMagiQuery->getMessage(), "</br>";
                        }
