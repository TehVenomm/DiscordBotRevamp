<?php
    function insertHealMagi() {
        ...
        if (($handle = fopen($linkHealMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $healMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $healMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $healMagiQuery) 
                        {
                            echo 'Caught exception: ',  $healMagiQuery->getMessage(), "</br>";
                        }
