<?php
    function insertFireMagi() {
        ...
        if (($handle = fopen($linkFireMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $fireMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $fireMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $fireMagiQuery) 
                        {
                            echo 'Caught exception: ',  $fireMagiQuery->getMessage(), "</br>";
                        }
