<?php
    function insertWaterMagi() {
        ...
        if (($handle = fopen($linkWaterMagi, "r")) !== FALSE) {
        ...
        foreach($spreadsheet_data as $rowcount){
            if($skip2rowscounter > 1){
                ...
                if (!empty($MagiDescCSV) || $MagiNameCSV == "Name")
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $waterMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $waterMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $waterMagiQuery) 
                        {
                            echo 'Caught exception: ',  $waterMagiQuery->getMessage(), "</br>";
                        }
