<?php
    function insertDarkMagi() {
        ...
        if (($handle = fopen($linkDarkMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $darkMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $darkMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $darkMagiQuery) 
                        {
                            echo 'Caught exception: ',  $darkMagiQuery->getMessage(), "</br>";
                        }
