<?php
    function insertSupportMagi() {
        ...
        if (($handle = fopen($linkSupportMagi, "r")) !== FALSE) {
                ...
                if (!empty($MagiDescCSV))
                {
                    if($qty == 0)
                    {
                        $sql = "INSERT INTO `magitable`
                                VALUES ('', ?, ?, ?, ?, ?)"; //DARK
                        $supportMagiQuery = $conex -> prepare($sql);
                        try
                        {
                            $supportMagiQuery -> execute(array($MagiNameCSV, $MagiCDCSV, $MagiDescCSV, $MagiObsCSV, $idMagiType));
                        } 
                        catch (Exception $supportMagiQuery) 
                        {
                            echo 'Caught exception: ',  $supportMagiQuery->getMessage(), "</br>";
                        }
